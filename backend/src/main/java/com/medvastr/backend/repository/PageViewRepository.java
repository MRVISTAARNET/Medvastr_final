...
public interface PageViewRepository extends JpaRepository<PageView, Long> {
    ...
    @Query("SELECT COUNT(pv) FROM PageView pv WHERE COALESCE(pv.viewTime, pv.session.startTime) >= :start AND COALESCE(pv.viewTime, pv.session.startTime) <= :end")
    ...
    @Query("SELECT pv.pageUrl, pv.pageTitle, COUNT(pv), COUNT(DISTINCT pv.session.visitorId), COALESCE(AVG(pv.durationSeconds), 0), SUM(CASE WHEN pv.entry = true THEN 1 ELSE 0 END), SUM(CASE WHEN pv.exit = true THEN 1 ELSE 0 END) " +
           "FROM PageView pv WHERE COALESCE(pv.viewTime, pv.session.startTime) >= :start AND COALESCE(pv.viewTime, pv.session.startTime) <= :end GROUP BY pv.pageUrl, pv.pageTitle ORDER BY COUNT(pv) DESC")
